...
async fn add_security_headers(mut res: Response<BoxBody>) -> Result<Response<BoxBody>, Infallible> {
    ...
    headers.append(
        ...
        HeaderValue::from_static("default-src 'none'; manifest-src 'self'; connect-src 'self' https://analytics.umami.is; script-src 'self' https://analytics.umami.is; style-src 'self'; img-src 'self' https://opusclassical.zunh.nl-ams1.upcloudobjects.com"),
    );
    headers.append(
