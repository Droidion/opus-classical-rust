...
pub async fn composer_handler(
    ...
    let composer: Composer = database.get_composer(slug.into_inner().as_str()).await.map_err(handle_error)?;
    let genres: Vec<Genre> = database.get_genres(composer.id).await.map_err(handle_error)?;
